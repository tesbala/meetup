...
const TABS: {id:EventTab; label:string; icon:React.ReactNode}[] = [
  { id:"created", label:"Created by me", icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> },
  { id:"joined",  label:"Joined",        icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
  { id:"saved",   label:"Saved",         icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z"/></svg> },
  { id:"past",    label:"Past events",   icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
];

export default function MyEventsPage() {
  const router = useRouter();

  // ── Step 1: get uid from cookie ─────────────────────────────────────────
  ...
  // ── Step 2: load summary once uid is ready ──────────────────────────────
  useEffect(() => {
    if (!uid) return;
    loadMyEventsSummary(uid).then(res => {
      if (res.success && res.data) setSummary(res.data);
    });
  }, [uid]);

  // ── Step 3: load tab data once uid is ready ─────────────────────────────
  useEffect(() => {
    ...
    const loaders: Record<EventTab, () => Promise<{ success: boolean; error?: string; data?: MyEvent[] }>> = {
      created: () => loadCreatedEvents(uid),
      joined:  () => loadJoinedEvents(uid),
      saved:   () => loadSavedEvents(uid),
      past:    () => loadPastEvents(uid),
    };
  ...
  }, [uid, activeTab]); // runs when uid is set OR tab changes

  // ── When tab changes, show loading if not already fetched ───────────────
  useEffect(() => {
    if (!fetched.has(activeTab)) setLoading(true);
  }, [activeTab]);
  ...
  // ── Cancel / Delete handlers — now pass uid ─────────────────────────────
  const handleCancel = async () => {
    if (!cancelId || !uid) return;
    const res = await cancelEvent(cancelId, uid);
    if (res.success) {
      setCreated(prev => prev.map(e => e.id === cancelId ? { ...e, status:"cancelled" as const } : e));
    }
    setCancelId(null);
  };
  ...
  const handleDelete = async () => {
    ...
    const res = await deleteEvent(deleteId, uid);
    if (res.success) {
  ...
  // Don't render until uid is known (prevents flash)
  if (!uid) return null;
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#F5F5FA", minHeight:"100vh", width:"100%" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .tabs-bar { display:flex; gap:6px; margin-bottom:16px; overflow-x:auto; scrollbar-width:none; padding-bottom:2px; }
        ...
        @media (max-width:580px) {
          ...
          .cc-actions { grid-column:1/-1; flex-direction:row !important; flex-wrap:wrap; padding:0 12px 12px !important; min-width:0; gap:7px; }
        ...
        @media (max-width:580px) {
          ...
          .ac-actions { grid-column:1/-1; flex-direction:row !important; padding:0 12px 12px !important; min-width:0; gap:7px; }
      ...
      <div className="cc-grid">
        ...
        <div style={{ padding:"13px 14px",minWidth:0 }}>
          ...
          <div style={{ display:"flex",gap:"12px",flexWrap:"wrap",marginBottom:"7px" }}>
            <span style={{ fontSize:"11px",color:"#888780",display:"flex",alignItems:"center",gap:"3px" }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#B4B2A9" strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg>{e.dateDisplay} · {e.time}</span>
            <span style={{ fontSize:"11px",color:"#888780",display:"flex",alignItems:"center",gap:"3px" }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#B4B2A9" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>{e.city}, {e.state}</span>
          </div>
          {e.max ? (
          ...
          {e.status!=="cancelled" && (
            <div className="analytics-row">
              <span style={{ fontSize:"11px",color:"#888780",display:"flex",alignItems:"center",gap:"4px" }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#B4B2A9" strokeWidth="2" strokeLinecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>{(e.views||0).toLocaleString()} views</span>
              {(e.revenue||0)>0 && <span style={{ fontSize:"11px",color:"#1D9E75",fontWeight:500,display:"flex",alignItems:"center",gap:"4px" }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="2" strokeLinecap="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>₹{(e.revenue||0).toLocaleString()} earned</span>}
            </div>
          )}
        </div>
        <div className="cc-actions">
          {e.status!=="past" && e.status!=="cancelled" && (<>
            ...
            <Link href={`/editEvent/${e.id}`}       className="action-btn" style={{ display:"block",padding:"7px 13px",background:"#F5F5FA",border:"1px solid #E8E8F0",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"#444441",textDecoration:"none",whiteSpace:"nowrap" }}>Edit</Link>
          ...
          {e.status==="cancelled" && <button onClick={onDelete} className="action-btn" style={{ padding:"7px 13px",background:"#FCEBEB",border:"1px solid #F09595",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"#A32D2D",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap" }}>Delete</button>}
          {e.status==="past" && (<>
            <Link href={`/editEvent/${e.id}`}        className="action-btn" style={{ display:"block",padding:"7px 13px",background:"#F5F5FA",border:"1px solid #E8E8F0",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"#444441",textDecoration:"none",whiteSpace:"nowrap" }}>Recap</Link>
